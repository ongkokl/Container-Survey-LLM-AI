-- Backfill GP.xlsx repair rules after the Phase 5 source correction.
--
-- Some environments may already have migration 0011 recorded as applied from
-- the earlier tariff-based draft. Wrangler does not rerun a migration merely
-- because its file contents later changed. This new migration re-applies the
-- corrected GP.xlsx repair-code master and exact GP/PAA component+damage+repair
-- relationships under a new migration number.
--
...
-- Remove the obsolete component-only repair table if it exists from the
-- superseded tariff-based draft. Runtime repair lookup now uses
-- component_damage_repair_rules exclusively.
DROP TABLE IF EXISTS component_repair_rules;
