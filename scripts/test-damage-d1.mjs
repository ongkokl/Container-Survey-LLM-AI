...
const container=id("container"), gate=id("gate"), survey=id("survey"), finding=id("finding"), run=id("run"), prediction=id("prediction"), decision=id("decision");
const now=new Date().toISOString();

console.log("Seeding isolated GP/PAA damage-flow fixture...");
sql(`
INSERT INTO containers (id,container_no,owner_code,equipment_category,serial_no,check_digit,latest_iso_size_type,created_at,updated_at)
VALUES ('${q(container)}','TSTU0000015','TSTU','U','000001','5','22G1','${now}','${now}');
INSERT INTO gate_cycles (id,container_id,cycle_sequence,gate_in_at,observed_iso_code,observed_container_type,observed_length_ft,observed_height_description,status,created_at)
VALUES ('${q(gate)}','${q(container)}',1,'${now}','22G1','GP',20,'8ft 6in','SURVEYING','${now}');
INSERT INTO surveys (id,gate_cycle_id,status,started_at,created_at,updated_at)
VALUES ('${q(survey)}','${q(gate)}','SURVEYING','${now}','${now}','${now}');
INSERT INTO findings (id,survey_id,finding_sequence,status,final_component_code,created_at,updated_at)
VALUES ('${q(finding)}','${q(survey)}',1,'REVIEW_REQUIRED','PAA','${now}','${now}');
`);
...
sql(`
INSERT INTO ai_runs (id,survey_id,finding_id,task_type,request_context_json,response_json,started_at,completed_at)
VALUES ('${q(run)}','${q(survey)}','${q(finding)}','DAMAGE_CLASSIFICATION','{"model":"mock-qwen"}','{}','${now}','${now}');
...
INSERT INTO prediction_candidates (id,prediction_id,candidate_code,rank,confidence,evidence_json)
VALUES ('${id("candidate")}','${q(prediction)}','DT',1,0.92,'{}');
...
sql(`
INSERT INTO surveyor_decisions (id,finding_id,prediction_id,field_type,ai_value,final_value,decision,created_at)
VALUES ('${q(decision)}','${q(finding)}','${q(prediction)}','DAMAGE','DT','GD','CORRECTED','${now}');
...
JOIN ai_runs r ON r.finding_id=f.id
JOIN ai_predictions p ON p.ai_run_id=r.id AND p.prediction_type='DAMAGE'
JOIN surveyor_decisions sd ON sd.prediction_id=p.id AND sd.field_type='DAMAGE'
WHERE f.id='${q(finding)}';
`)?.[0]?.results?.[0];
